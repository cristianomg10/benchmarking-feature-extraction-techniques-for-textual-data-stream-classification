        ...
        for instancesNumber in instances_test:
            ...
            for i in range(number_executions):
                ...
                if env == "twitter":
                    ...
                    df_ = pd.read_csv("datasets/amazon-reviews-fixed.csv", encoding='latin-1')
                    df_["polarity"] = df_["polarity"].map({1: 0, 2: 1})
                    os.unlink(f"{folder_name}/amazon-reviews-fixed.csv")
                    df_[["text", "polarity"]].sample(instancesNumber).to_csv(f"{folder_name}/amazon-reviews-fixed.csv", index=False)
